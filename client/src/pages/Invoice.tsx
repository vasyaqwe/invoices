...
export const Invoice = () => {
    ...
    const paymentTerms = new Date(paymentTermsDate.setDate(date.getDate() + +invoice?.paymentTerms.match(/\d+/)![0]))

    const allItemsTotal = invoice?.items.reduce((a: number, b: Item) => a + (b.price * b.quantity), 0)
    ...
    return (
        isLoading || !id ? <Spinner /> :
            <>
                ...
                <Link className='flex items-center gap-3' to="/"><Chevron /> Back</Link>
                ...
                <section className='mt-4 bg-primary-800 rounded-md p-6 text-neutral-400 font-thin grid gap-12 mb-32'>
                    <div className='flex justify-between items-start gap-4'>
                        <div>
                            <p className='font-semibold text-xl'><span className="text-neutral-500">#</span>{id?.toUpperCase()}</p>
                    ...
                    <div className='flex justify-between items-start flex-wrap gap-12 max-w-[90%]'>
                        <div>
                            <p>Invoice Date</p>
                            <p className='text-xl text-white'><b>{formatDate(date)}</b></p>
                            <p className='mt-7'>Payment Due</p>
                            <p className='text-xl text-white'><b>{formatDate(paymentTerms)}</b></p>
                        </div>
                        <div>
                            <p>Bill To</p>
                            <p className='text-xl text-white'><b>{invoice.billTo.clientName}</b></p>
                        ...
                        <div>
                            <p>Sent to</p>
                            <p className='text-xl text-white'><b>{invoice.billTo.clientEmail}</b></p>
                        </div>
                    ...
                    <table className='bg-primary-600 rounded-md overflow-hidden'>
                        <thead>
                            <tr>
                                <th className='text-left pt-7 px-7 hidden sm:table-cell'>Item Name</th>
                                <th className='text-center pt-7 px-7 hidden sm:table-cell'>QTY.</th>
                                <th className='text-left pt-7 px-7 hidden sm:table-cell' >Price</th>
                                <th className='text-right pt-7 px-7 hidden sm:table-cell'>Total</th>
                            </tr>
                        </thead>
                        <tbody>
                            {invoice.items.map((item: Item) => (
                                <tr key={item.id} className='font-semibold text-white'>
                                    <td className='sm:text-left py-5 px-7 sm:p-7 block sm:table-cell'>
                                        <span className='text-neutral-400 font-thin sm:hidden'>Item Name: </span>
                                        {item.name}</td>
                                    <td className='sm:text-center py-5 px-7 sm:p-7 block sm:table-cell'>
                                        <span className='text-neutral-400 font-thin sm:hidden'>QTY.: </span>
                                        {item.quantity}</td>
                                    <td className='sm:text-left py-5 px-7 sm:p-7 block sm:table-cell'>
                                        <span className='text-neutral-400 font-thin sm:hidden'>Price: </span>
                                        {formatCurrency(item.price)}</td>
                                    <td className='sm:text-right py-5 px-7 sm:p-7 block sm:table-cell'>
                                        <span className='text-neutral-400 font-thin sm:hidden'>Total: </span>
                                        {formatCurrency(itemsTotal(item))}</td>
                                </tr>
                            ))}
                            ...
                            <tr className='font-semibold text-white'>
                                <td className='sm:text-left py-5 px-7 sm:p-7 '>
                                    ...
                                    <span className='sm:text-right py-5 px-7 sm:p-7 text-2xl sm:text-3xl sm:hidden'>
                                        {formatCurrency(allItemsTotal)} </span>
                                </td>
                                <td className='sm:text-left py-5 px-7 sm:p-7 hidden sm:table-cell'>
                                </td>
                                <td className='sm:text-left py-5 px-7 sm:p-7 hidden sm:table-cell'>
                                </td>
                                <td className='sm:text-right py-5 px-7 sm:p-7 text-2xl sm:text-3xl hidden sm:block'>
                                    {formatCurrency(allItemsTotal)}</td>
                            </tr>
                        </tfoot>
                    </table>
                </section>
