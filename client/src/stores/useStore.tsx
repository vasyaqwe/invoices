...
export const useStore = create<StoreState>()(
    devtools(
        (set, get) => ({
            ...
            onCurrentInvoiceChange: (currentInvoice) => set(() => ({ currentInvoice })),
            onBackdropClick: () => {
                const target = Object.keys(get().modals).find(v => get().modals[v])
                if (target) set((state) => ({ modals: { ...state.modals, [target]: false } }))
            },
