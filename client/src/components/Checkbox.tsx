...
export const Checkbox = ({ onChange, checked, label, id, disabled = false }: {
    ...
    return (
        <div className="flex flex-wrap items-center gap-2">
            <button disabled={disabled} aria-checked={checked} type="button" id={id}
                onClick={() => onChange(!checked)} data-state={checked ? 'checked' : 'unchecked'}
     ...
     focus-visible:outline-none 
        ...
        disabled:opacity-50 data-[state=checked]:bg-accent-400 ">
                {checked && <svg className={`w-[10px] h-[11px] pointer-events-none`}
